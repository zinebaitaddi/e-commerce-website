...
/** ----------- Slider ----------- */
let imgs = ["img/Slider/1.jpg", "img/Slider/2.jpg"];
...
let productImg = document.querySelector("#overlay-img");  // L'élément d'image du slider
...
fetch(`https://json-server-brown-five.vercel.app/products?id=${id}`)
  .then((res) => res.json())
  .then((data) => {
    // Vérifie si le produit a bien des images
    if (data[0].img) {
      productImg.src = data[0].img;  // Met à jour l'image du slider avec celle du produit
    } else {
      console.log("Pas d'image disponible pour ce produit.");
    }
  })
  .catch((error) => {
